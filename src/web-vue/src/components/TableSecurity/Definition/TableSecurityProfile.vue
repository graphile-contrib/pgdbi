<template>
    <v-container ma-0 pa-0 :class="myClass">
      ...
      <v-row >
        <v-col v-if="policies.length > 0" cols="6">
          <v-data-table
            :items="policies"
            :headers="headers"
            hide-default-footer
...
<script>
  ...
  import TableRlsPolicyDialog from '@/components/TableSecurity/Dialogs/TableRlsPolicyDialog.vue'

  export default {
    name: 'TableSecurityProfile',
    components: {
      // TableGrantGrid,
      TablePolicyDefinitionGrants,
      TableRlsPolicyDialog
    },
    props: {
      policyDefinition: {
        type: Object,
    ...
    computed: {
      entityName () {
        return this.table ? `${this.table.tableSchema}.${this.table.tableName}` : 'abstract'
      ...
      policies () {
        console.log(this.policyDefinition)
        return this.policyDefinition.rlsQualifiers || []
