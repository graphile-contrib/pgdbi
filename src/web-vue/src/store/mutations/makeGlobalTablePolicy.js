function makeGlobalTablePolicy(state, payload) {
  ...
  if (existing) {
    ...
    const policyDefinition = state.policies.find(
      p => p.id === policyDefinitionId,
    );
    const otherPolicies = state.policies.filter(
      p => p.id !== policyDefinition.id,
    );
    ...
    state.policies = [...otherPolicies, ...[newGlobalPolicy]].sort(function(
      a,
      b,
    ) {
      return a.id < b.id;
